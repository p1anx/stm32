...
static void TIM_Mode_Config(void)
{
	...
	htimx.Instance = ADVANCE_TIM;
	/* 累计 TIM_Period个后产生一个更新或者中断*/		
	//当定时器从0计数到999，即为1000次，为一个定时周期
	htimx.Init.Period = TIMx_PSC; // 高级控制定时器时钟源TIMxCLK = HCLK=168MHz 
	// 设定定时器频率为=TIMxCLK/(TIM_Prescaler+1)=1MHz
	htimx.Init.Prescaler = TIMx_ARR;	
	// 采样时钟分频
	...
	//初始化通道1输出PWM 
	HAL_TIM_PWM_ConfigChannel(&htimx,&TIM_OCInitStructure,TIM_CHANNEL_x);
	...
	HAL_TIMEx_ConfigBreakDeadTime(&htimx, &TIM_BDTRInitStructure);
