...
/***************************************************************************************
  ...
  ***************************************************************************************
  */
// for HAL, commnet below and add "gpio.h" for hal
...
void OLED_GPIO_Init(void)
{
	...
	// stm32 gpio is initialized by HAL lib
	OLED_HAL_GPIO_Init();
...
void OLED_ShowStr(int16_t X, int16_t Y, char *String)
{ 
	uint8_t FontSize = OLED_8X16;
	uint16_t i = 0;
	char SingleChar[5];
	uint8_t CharLength = 0;
	uint16_t XOffset = 0;
...
void OLED_ShowNumInt(int16_t X, int16_t Y, uint32_t Number)
{
	uint8_t i;
	uint8_t Length;
	uint8_t FontSize  = FONT_SIZE;
	if(Number/10==0){
		Length = 1;
	}
	...
	else if(Number/10000 > 0 && Number/100000 == 0)
	{
		Length = 5;
	}
	else if(Number/100000 > 0 && Number/1000000 == 0)
	{
		Length = 6;
	}
	else if(Number/1000000 > 0 && Number/10000000 == 0)
	{
		Length = 7;
	}
	else
